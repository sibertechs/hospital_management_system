...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['send_message'])) {
    ...
    // Insert the message into the units_messages table, marking it as sent by doctor
    $query = "INSERT INTO units_messages (message, created_at, doctor_id, unit, sender_type) 
              VALUES ('$message', NOW(), '$doctor_id', '$unit', 'doctor')";
...
// Fetch messages sent by the doctor and messages sent to the doctor
$query_messages = "
    SELECT message, created_at, unit, sender_type,
           CASE
               WHEN sender_type = 'doctor' THEN 'Sent to'
               ELSE 'Received from'
           END AS message_direction
    FROM units_messages 
    WHERE (doctor_id = '$doctor_id' AND sender_type = 'doctor')
       OR (unit = 'Doctor' AND sender_type != 'doctor')
...
<head>
    ...
    <title>Doctor to Unit Communication</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/tailwindcss/2.2.19/tailwind.min.css">
</head>
<body class="bg-gray-100">
    <div class="flex">
        <?php require "./public/include/sidebar_doctor.php"; // Include sidebar ?>
        
        <main class="flex-1 p-8">
            <div class="container p-4 mx-auto">
                <div class="max-w-3xl p-6 mx-auto bg-white rounded-lg shadow-md">
                    <h1 class="mb-4 text-xl font-bold">Communicate with Departments - <?php echo htmlspecialchars($doctor['name']); ?></h1>
