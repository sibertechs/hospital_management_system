...
// Fetch prescription data with patient names
$query = "
    SELECT p.id, pr.name, p.drug_name, p.dosage, p.created_at
    FROM prescriptions p
    INNER JOIN patient_registration pr ON p.patient_id = pr.id
"; // Adjust the table names and column names as needed
       ...
       <main class="flex-1 p-8 ml-4">
        <div class="container mx-auto">
            <div class="max-w-6xl p-6 mx-auto bg-white rounded-lg shadow-md">
                <h1 class="mb-4 text-2xl font-bold">Prescriptions List</h1>
                ...
                <table class="min-w-full bg-white border border-gray-300">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="p-4 border-b"><?php echo htmlspecialchars($row['name'] ?? 'N/A'); ?></td>
                            <td class="p-4 border-b"><?php echo htmlspecialchars($row['drug_name'] ?? 'N/A'); ?></td>
                            <td class="p-4 border-b"><?php echo htmlspecialchars($row['dosage'] ?? 'N/A'); ?></td>
                            <td class="p-4 border-b"><?php echo htmlspecialchars($row['created_at'] ?? 'N/A'); ?></td>
                        </tr>
                        <?php endwhile; ?>
                    </tbody>
                </table>
